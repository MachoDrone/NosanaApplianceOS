...
def find_boot_files(work_dir):
    """Find the correct boot files for Ubuntu 24.04.2"""
    boot_files = {
        'eltorito': "",
        'boot_cat': "",
        'efi_boot': ""
    ...
    # Look for eltorito.img
    eltorito_paths = [
        os.path.join(work_dir, "boot", "grub", "i386-pc", "eltorito.img"),
        os.path.join(work_dir, "isolinux", "isolinux.bin"),
        os.path.join(work_dir, "boot", "isolinux", "isolinux.bin")
    ]
    
    for path in eltorito_paths:
        if os.path.exists(path):
            boot_files['eltorito'] = os.path.relpath(path, work_dir)
            break
    
    # Look for boot.cat
    boot_cat_paths = [
        os.path.join(work_dir, "boot", "grub", "boot.cat"),
        os.path.join(work_dir, "isolinux", "boot.cat"),
        os.path.join(work_dir, "boot", "boot.cat")
    ]
    
    for path in boot_cat_paths:
        if os.path.exists(path):
            boot_files['boot_cat'] = os.path.relpath(path, work_dir)
            break
    
    # Look for EFI boot image
    efi_paths = [
        os.path.join(work_dir, "boot", "grub", "efi.img"),
        os.path.join(work_dir, "EFI", "ubuntu", "grubx64.efi"),
        os.path.join(work_dir, "EFI", "boot", "bootx64.efi")
    ]
    
    for path in efi_paths:
        if os.path.exists(path):
            boot_files['efi_boot'] = os.path.relpath(path, work_dir)
            break
...
def remaster_ubuntu_2204(dc_disable_cleanup=False, inject_hello=True, inject_autoinstall=True):
    ...
    try:
        # Extract ISO file tree using 7z (more reliable than dd approach)
        ...
        # Find boot files
        boot_files = find_boot_files(work_dir)
        print(f"Found boot files: {boot_files}")
        ...
        # Create ISO using Ubuntu 24.04.2 compatible method
        if boot_files['eltorito'] and os.path.exists(os.path.join(work_dir, boot_files['eltorito'])):
            # Use the simplified approach for Ubuntu 24.04.2
            create_iso_cmd = f"""
            xorriso -as mkisofs \\
                ...
                -J -joliet-long \\
                -cache-inodes \\
                -b {boot_files['eltorito']} \\
                -c {boot_files['boot_cat'] or 'boot.cat'} \\
                -no-emul-boot \\
                -boot-load-size 4 \\
                -boot-info-table \\
                -eltorito-alt-boot \\
                -e EFI/ubuntu/grubx64.efi \\
                -no-emul-boot \\
            ...
            # Fallback approach - create a basic ISO
            create_iso_cmd = f"""
            xorriso -as mkisofs \\
                -r -V "NosanaAOS Ubuntu 24.04.2" \\
                -o {new_iso_filename} \\
        ...
        if not run_command(create_iso_cmd, "Creating new ISO"):
            print("First attempt failed, trying alternative method...")
            
            # Alternative method using genisoimage if available
            alt_cmd = f"""
            genisoimage -r -V "NosanaAOS Ubuntu 24.04.2" \\
                -cache-inodes -J -l \\
                -o {new_iso_filename} \\
                {work_dir}
            """
            
            if not run_command(alt_cmd, "Creating ISO with genisoimage", check=False):
                print("✗ Both ISO creation methods failed")
                return False
