...
1. **Web-UI Security**  
   ...
   • No extra login step is required by default.  
   • Operators may optionally enable a simple **local pass-phrase** in the web settings for extra peace of mind (stored as a bcrypt hash on the writable partition).

2. **Text-Menu (TUI) Main Screen**  
   _Metrics shown at a glance_  
   • GPU utilisation & temperature (per card)  
   • Nosana jobs queue depth & current job status  
   • Active Docker containers count  
   • CPU & RAM usage  
   • Network throughput (last 5 min avg)  
...
3. **Solana Wallet Backup & Restore**  
   • Documented procedure included in the user manual: copy `~/.nosana/nosana_key.json` to an **encrypted USB drive** or export it as an **encrypted QR code** generated by the TUI.  
   • Operators are reminded that Nosana never stores or sees this key.

4. **Immutable OS Hardening**  
   • Root filesystem is read-only, signed, and updated atomically (A/B).  
   • **Developer mode** switch available in the TUI: when enabled, the appliance remounts `/` read-write until next reboot.  
   • Secure Boot & `dm-verity` enabled to guard against offline tampering.  
   • TPM/remote attestation left for future roadmap.
