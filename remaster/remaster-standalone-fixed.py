...
def verify_hello_files(new_iso):
    print("Verifying hello files in new ISO...")
    
    # Mount and check the ISO
    with tempfile.TemporaryDirectory() as temp_mount:
        result = subprocess.run(f"sudo mount -o loop {new_iso} {temp_mount}", shell=True, capture_output=True, text=True)
        if result.returncode == 0:
            hello_dir = os.path.join(temp_mount, "hello")
            if os.path.exists(hello_dir):
                hello_txt = os.path.join(hello_dir, "hello.txt")
                if os.path.exists(hello_txt):
                    with open(hello_txt, 'r') as f:
                        content = f.read()
                        if "Hello from NosanaAOS" in content:
                            print("✓ Hello files verified successfully")
                            subprocess.run(f"sudo umount {temp_mount}", shell=True, capture_output=True)
                            return True
            
            subprocess.run(f"sudo umount {temp_mount}", shell=True, capture_output=True)
        
        print("✗ Hello files verification failed")
        return False
...
def remaster_ubuntu_2204(dc_disable_cleanup=False, inject_hello=True, inject_autoinstall=True):
    ...
    try:
        ...
        if not run_command(f"dd if={iso_filename} of=boot_hybrid.img bs=512 count=1", "Extracting MBR template"):
            return False
        ...
        if os.path.exists(new_iso_filename):
            ...
            # Verify hello files if they were injected
            if inject_hello:
                verify_hello_files(new_iso_filename)
